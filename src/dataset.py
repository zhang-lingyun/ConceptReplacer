...
class Dataset(TorchDataset):
    def __init__(
        ...
        self.image_paths = sorted(glob(os.path.join(data_dir, "*.jpg")), key=extract_number)
        if len(self.image_paths) == 0:
            self.image_paths = sorted(glob(os.path.join(data_dir, "*.png")), key=extract_number)
        self.mask_paths = sorted(glob(os.path.join(data_dir, "*.npy")), key=extract_number)
