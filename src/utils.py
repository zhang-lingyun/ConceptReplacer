...
def apply_blur(image, mask, blur_kernel_size=15):
    # Apply Gaussian blur from torchvision
    blurred_image = TF.gaussian_blur(image, kernel_size=(blur_kernel_size, blur_kernel_size))
    return blurred_image * mask + image * (1 - mask)

def get_colored_segmentation_with_blur(mask, boundry_mask, image, colors, blur_kernel_size=31):
    boundry_mask_rgb = 0
    if boundry_mask is not None:
    ...
    # Convert mask to a binary mask for blurring
    blur_mask = (mask > 0).float()  # Make mask binary: 1 where mask > 0, 0 elsewhere
    # blurred_image = apply_blur(image, blur_mask, blur_kernel_size)
    
    # Apply boundary and mask blending as before
    if boundry_mask is not None:
        # return (boundry_mask_rgb * 0.6 + mask_rgb * 0.3 + blurred_image * 0.4).permute(1, 2, 0)
        return (boundry_mask_rgb * 0.6 + mask_rgb * 0.3).permute(1, 2, 0)
    else:
        return (mask_rgb * 0.6 + image * 0.4).permute(1, 2, 0)