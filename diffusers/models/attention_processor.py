...
class LoRAAttnProcessor(nn.Module):
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, scale=1.0):
        ...
        location_key = attn.head_to_batch_dim(location_key)
        key = attn.head_to_batch_dim(key)
        value = attn.head_to_batch_dim(value)

        attention_probs, _ = attn.get_attention_scores(query, key, attention_mask)
        hidden_states = torch.bmm(attention_probs, value)
        hidden_states = attn.batch_to_head_dim(hidden_states)
...
class LoRAXFormersAttnProcessor(nn.Module):
    def __init__(self, hidden_size, cross_attention_dim, rank=4, attention_op: Optional[Callable] = None):
        ...
        self.to_q_lora = nn.Linear(hidden_size, hidden_size, bias=False)
        self.to_k_lora = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.to_v_lora = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
